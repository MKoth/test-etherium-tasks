...
const cETH = new web3.eth.Contract(ctokenAbi, "0x4Ddc2D193948926D02f9B1fE9e1daa0718270ED5")

function getCEthMintEvents(startBlock, endBlock) {
  return cETH.getPastEvents("Mint",{fromBlock:startBlock, toBlock:endBlock})
}

function getAllMintEvents(from, to){
  batch = 10000;
  let promisesArray = [];
  for(i=from; i<to; i+=batch){
    //console.log('from'+i+", to"+(i+batch<to?i+batch:to) );
    promisesArray.push(getCEthMintEvents(i, i+batch<to?i+batch:to));
  }
...
function calculateBiggestMintAmount(data, exchangeRate){
  let biggestMintAmount = 0;
  let address = '';
  data.forEach(dataArr=>{
    dataArr.forEach(dataArrInner=>{
      if(biggestMintAmount<dataArrInner.returnValues.mintAmount){
        biggestMintAmount = dataArrInner.returnValues.mintAmount;
        address = dataArrInner.address;
      }
      
    })
  });

  //const exchangeRate = await getUsdEtheriumExchangeRate();

  //console.log(exchangeRate);

  biggestMintAmount = new BigNumber(biggestMintAmount);

  return {mintAmountInEth: biggestMintAmount.dividedBy(new BigNumber(1e+18)).toFixed(), mintAmountInUSD: biggestMintAmount.dividedBy(new BigNumber(1e+18)).multipliedBy(new BigNumber(exchangeRate)), address}
}

//getCEthMintEvents(7710758, 8710758).then(res=>console.log(res));

app.get('/', function (req, res, next) {

  (async () => {
    
    const exchangeRate = await axios.get('https://min-api.cryptocompare.com/data/price?fsym=ETH&tsyms=USD');
    const data = await getAllMintEvents(7710758, 8710758);
    return res.json(calculateBiggestMintAmount(data, exchangeRate.data.USD));
  })().catch(next);
...
//let data = await getCEthMintEvents(7710758, 7710761);